...
@interface CJMAlertAction ()
@property (nonatomic, copy) void (^ __nullable handler)(CJMAlertAction *action);
...
@implementation CJMAlertAction

+ (instancetype)actionWithTitle:(nullable NSString *)title style:(UIAlertActionStyle)style handler:(void (^ __nullable)(UIAlertAction *action))handler {
    return [[CJMAlertAction alloc] initWithTitle:title style:style handler:handler];
}
...
@property (nonatomic, strong) NSMutableArray <CJMAlertAction *>*actions;
...
- (void)touchUpInsideButton:(UIButton *)button {
    ...
    CJMAlertAction *action = [_actions objectAtIndex:index];
    if (action.handler) {
